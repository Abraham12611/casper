...
export function CreditMeter() {
  ...
  const Casper = customer?.features?.Casper_credits as
  ...
  const remaining = typeof Casper?.balance === "number" ? Casper.balance : undefined;
  const limit = typeof Casper?.included_usage === "number" ? Casper.included_usage : undefined;
  const pct =
  ...
  return (
    <div className="w-full max-w-md border border-slate-200 dark:border-slate-800 rounded-md p-3">
      <div className="text-sm font-medium mb-2">Casper credits</div>
      {typeof pct === "number" ? (
