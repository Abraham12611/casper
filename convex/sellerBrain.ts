...
export const debugResetOnboarding = mutation({
  args: {},
  returns: v.null(),
  handler: async (ctx) => {
    const user = await authComponent.getAuthUser(ctx);
    ...
    // Find and delete all onboarding flows for this profile
    const flows = await ctx.db
      .query("onboarding_flow")
      .withIndex("by_agencyProfileId", (q) => q.eq("agencyProfileId", profile._id))
      .collect();

    for (const flow of flows) {
      // Find and delete all crawl pages for this flow
      const pages = await ctx.db
        .query("crawl_pages")
        .withIndex("by_flow", (q) => q.eq("onboardingFlowId", flow._id))
        .collect();
    ...
    console.log(`Debug: Reset onboarding for user ${user._id} and profile ${profile._id}`);
    return null;
  },
});
